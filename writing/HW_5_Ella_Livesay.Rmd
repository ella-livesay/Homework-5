...
ggplot() +
  ...
  geom_sf(data = homicides_denver, aes(color = race), alpha = 0.65,
          size = 1, shape = 16) +
...
homicides_baltimore <- st_as_sf(homicides_baltimore, coords = c("lon", "lat"))%>%
  st_set_crs(4269)

baltimore <- tracts(state = "MD", cb = TRUE, class = "sf") %>%
  filter(COUNTYFP == "510")
```